...
def _evaluate_audit(args,model,legacy,dataset,index_map,audit,device,*,gates_enabled:bool):
    ...
    with geometry_gates_enabled(model,enabled=gates_enabled):
        for index,item in enumerate(audit):
            ...
            result=_forward(model,legacy,batch,correct,record,device,grad=False); _release(model); result["sample"]=sample; result["target_raster"]=correct["action_raster"]; result["loss"]=result["fm"]; result["support_energy"]=result["energy"].reshape(1)
