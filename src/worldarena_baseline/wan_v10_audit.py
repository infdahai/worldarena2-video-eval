...
def _family_reasons(metrics: Mapping[str, Any], threshold: int) -> list[str]:
    reasons: list[str] = []
...
def evaluate_v10_gate(
    ...
    *,
    step250_receipt: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    reasons: list[str] = []
    if step == 50:
    ...
    elif step in (250, 500):
        threshold = 11 if step == 250 else 14
        ...
        limit = 0.0 if step == 250 else 0.05
        if not math.isfinite(position) or position <= limit:
            reasons.append(
                "position_improvement_not_positive"
                if step == 250 else "position_improvement_not_above_5_percent"
            )
        if not math.isfinite(velocity) or velocity <= limit:
            reasons.append(
                "velocity_improvement_not_positive"
                if step == 250 else "velocity_improvement_not_above_5_percent"
            )
        if metrics.get("relation_enabled_beats_zero") is not True:
            reasons.append("relation_enabled_does_not_beat_zero")
        if step == 500 and (not isinstance(step250_receipt, Mapping) or step250_receipt.get("pass") is not True):
            reasons.append("missing_passing_step250_receipt")
    else:
        raise ValueError("v10 gate step must be 50, 250, or 500")
